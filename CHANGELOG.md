...
- **`yt-nota --registry <stats|list|backfill>`** — inspeção e administração do registro. `list` filtra por `--channel`/`--status`/`--verdict`/`--limit`; `backfill` importa o `processados.json` legado (aceita `--dry-run` e `--from-file`). Path do banco sobrescrevível com `--db`.
...
- Testes: 25 novos (166 no total) — `tests/test_registry.py` cobre ciclo de vida, a regressão de 2026-07-21, veredicto inválido, `filter_new` acima do teto de variáveis do SQLite (1.200 ids) e a semântica do backfill (não sobrescreve linha real, ignora entradas malformadas, idempotente).
