...
- Drafts em `<vault>\30-Recursos\Literatura\Pipeline\_processar\` (a partir de 2026-06-04, era `_drafts/` antes)

Skill `/yt-sintese` processa os drafts e produz notas finais em:
- `<vault>\30-Recursos\Literatura\<dominio>\<canal>\` (segue 7 domínios do ROTEAMENTO)

Quando criar, mover, renomear ou deletar nota no vault, respeitar as regras invariantes:

1. **Ler antes:** `<vault>\30-Recursos\Sistema\REGRAS-VAULT.md` — perfil cognitivo do usuário, princípio Zettelkasten, 7 domínios, regras anti-entropia, regra dos 3 cliques.
2. **Aplicar destino:** `<vault>\30-Recursos\Sistema\ROTEAMENTO.md` — Tabela A (curadoria de fontes) decide o domínio do canal.

Em conflito entre instrução pontual e estas regras, perguntar ao mantenedor antes de quebrar. Para validar drift, rodar `/vault-checkup --validar` sob demanda.
