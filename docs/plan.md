...
User consome YouTube como fonte de aprendizado. O fluxo manual hoje (abrir vídeo → "Mostrar transcrição" → copiar → colar no Claude → pedir nota) funciona pra um vídeo e vira fricção em batch.
...
- **Remote:** `https://github.com/user-handle/yt-nota.git`
...
Skill global: `<user-home>\.claude\skills\yt-sintese\SKILL.md` (não vive no repo do projeto, vive no `~/.claude` global).
...
```bash
git clone https://github.com/user-handle/yt-nota.git <user-home>/00_projetos/yt-nota
...
Skill já está em `<user-home>\.claude\skills\yt-sintese\`. Disponível em qualquer sessão Claude Code.
...
- v0.1.0 (2026-05-16): scaffold inicial com integração Anthropic API direta. Pipeline rodou até a chamada da API mas barrou em billing da conta da User.
